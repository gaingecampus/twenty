export const buildMcpServerInstructions = (
  ...
  [
    ...
    `  ACTION:           http_request | send_email | draft_email | navigate_app | code_interpreter | search_help_center | register_file_on_record`,
    ...
    `  Saving a file on a CRM record (Files tab): learn_tools(["register_file_on_record"]) then execute_tool register_file_on_record`,
    `  with objectNameSingular, recordId, and either sourceFileId only (AgentChat upload — original filename is preserved server-side)`,
    `  or contentBase64+filename (raw bytes). Do NOT rename chat uploads. Do NOT use create_one_attachment / update_one_attachment`,
    `  for file registration, and never reassign an existing attachment by matching filename.`,
    ``,
