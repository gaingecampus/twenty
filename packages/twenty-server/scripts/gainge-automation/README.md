...
2.21.0 workspace upgrade가 담당자 자동 배정과 후속 처리 기록을 설치한다. 기업·고객·문의·계약의 빈 DRI에 작성자/이번 수정자를 배정하고, 기업·고객·문의는 기존 DRI가 다르면 공동 담당자로 중복 없이 추가한다. 계약 DRI는 실행 컨설턴트다. 계약은 2.22.0부터 수정자를 공동 실행 컨설턴트로 자동 추가하지 않는다. 같은 업그레이드에서 자동화가 만든 공동 실행 컨설턴트 연결(`createdBySource=SYSTEM`, `createdByName=CRM 담당자 자동 배정`)을 소프트 삭제하며, 사람이 추가한 연결은 유지한다. 복구는 해당 행의 `deletedAt`을 비우면 된다. 계정 연결이 정확히 하나인 MANUAL 행위만 배정한다. 기존 기록 소급 변경과 영구 삭제는 하지 않는다.
