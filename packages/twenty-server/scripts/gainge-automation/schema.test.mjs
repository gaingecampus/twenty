...
test('transactional DRI and durable events across four objects',async()=>{
 ...
 for(const t of AUTOMATION_TARGETS){await c.query(`CREATE TABLE "${ns}"."${tables[t.table]}" (id uuid PRIMARY KEY DEFAULT gen_random_uuid(),name text,"${t.dri}" uuid,"deletedAt" timestamptz,"createdBySource" text,"createdByWorkspaceMemberId" uuid,"createdByName" text,"updatedBySource" text,"updatedByWorkspaceMemberId" uuid,"updatedByName" text,"customStage" text,"onboardingStatus" text)`);await c.query(`CREATE TABLE "${ns}"."${tables[t.link]}" (id uuid DEFAULT gen_random_uuid(),"${t.parent}" uuid REFERENCES "${ns}"."${tables[t.table]}"(id),"guseongweonId" uuid,"deletedAt" timestamptz,"createdBySource" text,"createdByName" text)`);}
 ...
 assert.equal((await c.query(`SELECT count(*)::int AS n FROM "${ns}"."${tables[t.link]}" WHERE "${t.parent}"=$1`,[row.id])).rows[0].n,1);
