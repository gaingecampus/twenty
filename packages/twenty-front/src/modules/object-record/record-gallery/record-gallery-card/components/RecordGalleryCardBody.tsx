...
export const RecordGalleryCardBody = ({
  recordId,
  isRecordReadOnly,
}: RecordGalleryCardBodyProps) => {
  const { objectPermissions, objectMetadataItem } =
    useRecordGalleryContextOrThrow();
  ...
  const setRecordGalleryCardHoverPosition = useSetAtomComponentState(
  ...
  const handleMouseEnter = (index: number) => {
    setRecordGalleryCardHoverPosition(index);
  };
          ...
          <StopPropagationContainer key={recordField.id}>
            <FieldContext.Provider
              value={{
                ...
                triggerEvent: 'CLICK',
                anchorId: `${RECORD_GALLERY_CARD_INPUT_ID_PREFIX}-${recordId}-${correspondingFieldDefinition.metadata.fieldName}`,
